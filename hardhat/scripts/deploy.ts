...
async function main() {
  ...
  const deployContracts = ['nftContract'];
  ...
  for (const toDeployName of deployContracts) {
    ...
    const artifactsPath = join(
      processDir,
      `../src/assets/artifacts/contracts/${toDeployContract.artifactsPath}`
    );
    ...
    console.log(
      `${toDeployContract.name} Contract Deployed to: ${contract.address} in Network Id ${chainId} `
    );
  }
}
